...
function PaymentMethodBadge({ method }: { method: string }) {
  ...
  const m = map[method] ?? { label: method, cls: "bg-white/10 text-muted-foreground" };
              ...
              <tbody>
                {orders.map((order) => (
                  <tr key={order.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-3 text-center hidden sm:table-cell"><PaymentMethodBadge method={order.paymentMethod || 'card'} /></td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="glass-card rounded-2xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-muted-foreground">Forma de Pagamento</span><PaymentMethodBadge method={order.paymentMethod || 'card'} /></div>
        </div>
