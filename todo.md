...
- [x] Fix date year in PDF ticket generation
- [x] Fix date year in email templates
...
- [x] Restore 3-tab Excel export (Dashboard, Passageiros, Financeiro)
- [x] Tab 1 Dashboard: Total Passageiros, Taxa Conversão, Embarque, Datas
...
- [x] Create simplified passenger list view
- [x] Show only 3 columns: Nome, Ponto de Embarque, Dia que vai
- [x] Add filter by date
- [x] Add filter by boarding point
...
## PHASE 18: 🐛 ADMIN TICKET VIEW DATE BUG FIX (June 2, 2026)

### Bug Report
- Admin ticket view shows wrong travel date (off by one day)
- Example: Order 3900002 shows 05 de junho instead of 06 de junho
- Email and Lista de Embarque show correct date
- Root cause: Timezone offset with Date.UTC() + toLocaleDateString()

### Fix Implemented
- [x] Identified timezone issue in Ingresso.tsx formatDate function
- [x] Replaced Date.UTC() with manual date parsing
- [x] Implemented direct string formatting without timezone conversion
- [x] Support for all 12 months in Portuguese
- [x] Created comprehensive vitest tests (6 tests passing)
- [x] Tests verify no off-by-one errors for all dates

### Backfill Results
- [x] Created backfill script for corrupted dates
- [x] Ran backfill on all 295 orders
- [x] Result: 0 corrupted dates found (database is clean!)
- [x] 16 unfixable orders (likely data entry errors, not date format issues)

### Final Status
- ✅ Admin ticket view now shows correct dates
- ✅ No timezone offset issues
- ✅ All dates format as "DD de MMMM de YYYY"
- ✅ Vitest tests pass (6/6)
- ✅ Database backfill complete
- ✅ Ready for production deployment


## PHASE 19: 🚀 URGENT - MANUAL PASSENGER ADDITION TO LISTA DE EMBARQUE (June 2, 2026)

### Feature Requirements
- [ ] Create database table for manual passengers
- [ ] Implement backend tRPC procedures (create, read, delete)
- [ ] Add "Adicionar Passageiro" button to Lista de Embarque
- [ ] Create form modal with: nome, data, ponto embarque, pedido (optional)
- [ ] Visual distinction for manual passengers (tag or color)
- [ ] Integrate manual passengers into boarding list display
- [ ] Include manual passengers in Excel export
- [ ] Include manual passengers in print view
- [ ] Add delete functionality for manual passengers
- [ ] Write vitest tests
- [ ] Manual entries must NOT create orders or affect payments
- [ ] Manual entries must NOT affect existing automatic entries

### Status: IN PROGRESS
- Event starts June 05 - URGENT
- Workaround for multi-date order bug
- Operational control for last-minute additions


## PHASE 20: 🎯 URGENT - MANUAL PASSENGER ADDITION FEATURE

- [x] Create `manual_passengers` table in database schema
- [x] Implement tRPC procedures for CRUD operations (create, listByEvent, listByEventAndDate, delete)
- [x] Create `AddManualPassengerModal` component with form fields
- [x] Add "Adicionar Passageiro" button to Lista de Embarque page
- [x] Integrate manual passengers into filtered boarding list
- [x] Add visual distinction (green background, MANUAL tag, delete button)
- [x] Update Excel export to include manual passengers via getPassengersForExport
- [x] Verify print view includes manual passengers (already supported)
- [x] Write and run vitest tests for manual passengers feature (6 tests passing)
- [x] Verify all TypeScript compilation passes

### Feature Summary
**Purpose**: Allow admin to manually add passengers to boarding list when multi-date orders don't populate correctly. This is a workaround for multi-date order issues and remains useful for operational needs (comps, last-minute additions).

**Key Characteristics**:
- Manual entries are visually distinct with green background and MANUAL tag
- Deletable via button in the list
- Included in Excel exports (marked as "manual" in orderStatus column)
- Included in print views
- NOT creating new orders or affecting payment data
- Purely operational for boarding verification
- Required fields: name, travelDate (YYYY-MM-DD), boardingPointId
- Optional fields: referenceOrderId, notes

**UI Components**:
- Modal form with fields: name, date (display-only), boarding point (dropdown), reference order (optional), notes (optional)
- Manual passengers displayed with green background and MANUAL tag
- Delete button (✕ Remover) visible only for manual entries
- Integrated into existing filter/search logic

**Database**:
- New table: `manual_passengers` with 9 columns
- Foreign keys: eventId, boardingPointId, createdBy
- Timestamp: createdAt (auto)

**API**:
- tRPC router: `admin.manualPassengers`
- Procedures: create, listByEvent, listByEventAndDate, delete
- All protected with adminProcedure

**Testing**:
- 6 vitest tests covering schema, CRUD operations, and integration
- All tests passing
